package io.mohajistudio.tangerine.prototype.infra.region.generator;
import io.mohajistudio.tangerine.prototype.infra.region.dto.AddressInfo;
import io.mohajistudio.tangerine.prototype.infra.region.dto.RegionDTO;
import io.mohajistudio.tangerine.prototype.infra.region.dto.Item;
...
@AllArgsConstructor
@Component
public class RepresentativeRegionGenerator {
...
//post 수정, 저장 만들때 같은 지역 입력받으면 400에러 보내도록 예외처리 요망
    ...
    public List<String> generate(List<RegionDTO> places){
    ...
    private List<String> generateForSinglePlace(RegionDTO place) {
        List<Item> items = place.getItems();
        if (!items.isEmpty()) {
            String address = items.get(0).getAddress();
            AddressInfo addressInfo = extracted(address);
    ...
    private static void generateForPlace(List<RegionDTO> places, Map<String, Integer> provinces, Map<String, Integer> cities, Map<String, Integer> districts) {
        for(RegionDTO place : places){
           List<Item> item = place.getItems();
    ...
    private static AddressInfo extracted(String address) {
